...
export const StakingTab = styled.div`
  ...
  /* border-bottom: ${(props) =>
    props.active ? "4px solid #EC4781" : "none"}; */
  cursor: pointer;
  position: relative;
...
export const IdoDetails = styled.div`
  ...
  display: flex;
  align-items: center;
  border: 1px solid #a73c6d;
  background: linear-gradient(113.14deg, #ff60aa -100%, #000000 150%);
...
export const Tag = styled.span`
  ...
  font-size: 16px;
  line-height: 28px;
  background: ${(props) => props.type === "interest" && "#536A66"};
  color: ${(props) => props.type === "interest" && "#67C8AB"};

  background: ${(props) => props.type === "time" && "#8F94FF"};
  color: ${(props) => props.type === "time" && "#Fff"};

  background: ${(props) => props.type === "default" && "#252022"};
  color: ${(props) => props.type === "default" && "#Fff"}; ;
`;
